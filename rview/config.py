...
# coding=UTF-8
#
# This file contains items that can be configured in rview.
#
...
class BaseConfig(object):# pylint: disable-msg=R0903
    ...
    LOG_FILE = LOG_ROOT + 'rview.log'
    LOG_LEVEL = logging.INFO
    LUCENE_INDEX_DIR = LUCENE_ROOT
    GROUPS = [
        {
            'name' : 'Test Images',
            'path' : '/var/www/rview/corpora',
            'description' : 'Test corpora supplied with rview.'
        }
    ]
...
class TestConfig(DevConfig):# pylint: disable-msg=R0903
    ...
    LOG_FILE = os.path.join(TEMP, 'rview.log')
...
class AnalyserConfig(BaseConfig):# pylint: disable-msg=R0903
    ...
    LOG_FORMAT = '[%(asctime)-15s %(levelname)-8s %(filename)-15s:'+\
                 '%(lineno)-5d %(funcName)-30s] %(message)s'
    LOG_FILE = LOG_ROOT + 'rview-analyser.log'

CONFIGS = {
    "default": 'rview.config.BaseConfig',
    "dev": 'rview.config.DevConfig',
    "test": 'rview.config.TestConfig',
    "analyser": 'rview.config.AnalyserConfig'
}
